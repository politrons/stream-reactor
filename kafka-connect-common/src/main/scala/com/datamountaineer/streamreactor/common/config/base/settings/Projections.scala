...
object Projections extends StrictLogging {
  def apply(kcqls: Set[Kcql],
            sourceConverterProps: Map[String, String],
  ...
  def getSourceConverters(
      kcql: Set[Kcql],
      props: Map[String, String]): Map[String, Converter] = {
    kcql
      ...
      .map({
        case (source, clazz) =>
          ...
          val converter = Try(Class.forName(clazz).newInstance()) match {
            ...
            case Failure(_) =>
              throw new ConfigException(
                s"Invalid KCQL is invalid for [$source]. [$clazz] should have an empty ctor!")
          }
